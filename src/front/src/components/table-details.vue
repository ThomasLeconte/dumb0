<script setup lang="ts">
  ...
  function getMostRecentDate(d1?: Date | string, d2?: Date | null) {
    const _d1 = d1 ? (d1 instanceof Date ? d1 : new Date(d1)) : null;
    const _d2 = d2 ? (d2 instanceof Date ? d2 : new Date(d2)) : null;

    if(!_d1 && !_d2) return null;
    if(!_d1 && _d2) return _d2;
    if(_d1 && !_d2) return _d1;
    return _d1!.getTime() > _d2!.getTime() ? _d1 : _d2;
  }
  ...
  const lastAnalyzeTooOld = computed(() => {
    ...
    const diff = new Date().getTime() - (lastAnalyze?.getTime() ?? 0);
