...
export class SqliteService {
    ...
    private static db: DatabaseInstance | null = null;
    private static dataDir: string | null = null;

    private static getDatabasePath(): string {
        if (!SqliteService.dataDir) {
            throw new Error('SqliteService not initialized. Call SqliteService.init(dataDir) first.');
        }
        return path.join(SqliteService.dataDir, 'app.db');
    }

    static getDatabase(): DatabaseInstance {
        if (!SqliteService.db) {
     ...
     * @param dataDir Dossier de stockage des donnees (userData Electron ou chemin serveur)
     * @throws {Error} Si le dossier ne peut pas etre cree
     */
    public static init(dataDir?: string): void {
        if (dataDir) SqliteService.dataDir = dataDir;
        if (!SqliteService.dataDir) {
            throw new Error('SqliteService not initialized. Call SqliteService.init(dataDir) first.');
        }

        if (!fs.existsSync(SqliteService.dataDir)) {
            try {
                fs.mkdirSync(SqliteService.dataDir, { recursive: true });
                console.log(`Dossier de donnees cree: ${SqliteService.dataDir}`);
            } catch (err) {
    ...
    static async createDatasource(args: any): Promise<void> {
        let form = args["form"] as CreateDatasourceFormDto;
